...
export default {
  ...
  mounted () {
    console.log('tableHeight:', window.innerHeight - 220)
    const dom = document.querySelector('.el-table__body-wrapper')
    let halfHeight = dom.scrollHeight / 2
    dom.addEventListener('scroll', (v) => {
      ...
      if (this.page === 1) {
        halfHeight = dom.scrollHeight / 2
      }
      console.log('halfHeight:', halfHeight)
      console.log('监听。。。', scrollDistance)
      if (scrollDistance <= halfHeight && this.page < this.totalPage) {
        this.getChaperList()
      }
      if (scrollDistance < 1 && this.page >= this.totalPage) {
